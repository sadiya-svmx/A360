...
export const normalizeData = function(
    ...
    data.forEach((row, index) => {
        // create new object
        const flatRow = getObjectWithoutKeys(row, ['children']);
        ...
        if (row.hasOwnProperty('children') || row.hasOwnProperty('detail')) {
            flatRow.hasChildren = true;
            flattenedData.push(flatRow);

            // validate children key
            ...
            } else if (flatRow.isExpanded && !hasChildrenContent && !flatRow.detail) {
...
export const normalizeColumns = function(columns) {
    ...
    if (Array.isArray(columns) && columns.length > 0) {
        const treeColumn = getObjectWithoutKeys(
            getSanitizedObject(columns[0], whitelistedColumnKeys),
            ['typeAttributes']
        );
        ...
        for (let i = 1; i < columns.length; i++) {
            normalizedColumns.push(
                getSanitizedObject(columns[i], whitelistedColumnKeys)
            );
        }
    }
