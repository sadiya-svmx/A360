...
import { keyCodes, isRTL } from 'c/utils';
...
export default class PrimitiveDatatableCell extends LightningElement {
    ...
    @track _mode = 'NAVIGATION';
    @track _currentInputIndex = 0;
    @track _internalTabIndex = -1;
    @track _actionableElementsCount;
     ...
     * @param {string} incomingMode is the new mode, 
     * needed because a cell can be in action mode but new mode being
    ...
    resetCurrentInputIndex (direction, incomingMode) {
        ...
        // in action mode till we call setMode above. 
        // So need to check new incoming mode too if action mode
    ...
    setFocusToActionableElement (activeInputIndex) {
        ...
        // navigate throught the datatable in ACTION mode, 
        // we can rememeber the active input position
    }
