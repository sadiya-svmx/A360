...
function createResizeSensor (
    ...
    return new ResizeSensor(
        resizeTarget,
        debounce(() => {
            // since this event handler is debounced,
            // it might be the case that at the time the handler is called,
            // the element is disconnected (this.hasDetachedListeners)
            // the scroll event which the ResizeSensor uses
            // can happen when table is hidden (as in console when switching tabs)
...
export class LightningDatatableResizeObserver {
    ...
    constructor (template, state, widthsData, columnWidthManager) {
        ...
        if (this._resizeObserverAvailable) {
            ...
            // fallback behavior for IE11 and Safari using existing
            // resize sensor functionality (less performant)
