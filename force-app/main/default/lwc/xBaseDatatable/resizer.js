import { normalizeBoolean, isRTL } from 'c/utils';
...
/**
 * It return the current widths for customer columns with object type
 * @param {object} state - table state
 * @param {object} widthsData - data regarding column and table widths
 * @returns {object} - the widths collection, every element
 * belong to a column with the same index in column prop
 */
export function getCustomerColumnWidthByObjectType (state, widthsData) {
    const columns = getColumns(state);
    const cols =  columns.reduce((prev, column, index) => {
        if (isCustomerColumn(column)) {
            prev.push(widthsData.columnWidths[index]);
        }
        return prev;
    }, []);
    const key = columns.reduce((prev, column) => {
        if (isCustomerColumn(column)
          && column.type === 'tree') {
            // eslint-disable-next-line no-param-reassign
            prev = column.ariaLabel;
        }
        return prev;
    }, []);
    return {
