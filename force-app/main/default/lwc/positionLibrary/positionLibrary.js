...
function createRelationship (originalConfig, disableReposition) {
    bindEvents();
...
function normalizeConfig (parent, config) {
    ...
    const flipConfig = checkFlipPossibility(
        config.overlay.isInside ? null : parent,
...
export function startPositioning (root, config, disableReposition) {
    ...
    // IMPORTANT: config.isInsideModal will be used 
    // to decide if element should use modal as viewport.
    ...
    return createRelationship(config, disableReposition);
}
