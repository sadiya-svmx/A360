import labelBInput from '@salesforce/label/c.Label_BInput';
import labelBlueAbbr from '@salesforce/label/c.Label_BlueAbbr';
import labelColorPickerInstructions from '@salesforce/label/c.Message_ColorPickerInstructions';
import labelErrorMessage from '@salesforce/label/c.Message_ColorPickerError';
import labelGInput from '@salesforce/label/c.Label_GInput';
import labelGreenAbbr from '@salesforce/label/c.Label_GreenAbbr';
import labelHexLabel from '@salesforce/label/c.Label_Hex';
import labelHueInput from '@salesforce/label/c.Label_Hue';
import labelRInput from '@salesforce/label/c.Label_RInput';
import labelRedAbbr from '@salesforce/label/c.Label_RedAbbrv';
import { LightningElement, api, track } from 'lwc';
import { guid, parseErrorMessage, keyCodes } from 'c/utils';
import {
    fullHexValue,
    hexToRgb,
    rgbToHex,
...
export default class LightningColorPickerCustom extends LightningElement {
    ...
    get gradientStyle () {
        return `background: ${this._hex ||
            'rgb(0, 85, 255)'}; position: relative;`;
    }
    ...
    handleHexChange (event) {
        ...
        if (isInputValid) {
            ...
            this._errorMessage = parseErrorMessage(event.srcElement.validity, {
    ...
    gradientCursorPosition (event) {
        ...
        /*
         * Caching the position x & y in the component so 
           that we can use it when moving the rainbow slider
    ...
    gradientCursorPositionFromKeydown (event) {
        ...
        /*
         * Caching the position x & y in the component 
           so that we can use it when moving the rainbow slider
         * instead of calculating the position of x & y each time.
         */
        this._cachePosition = { x, y };
        return { x, y };
    ...
    setCanvasCursor (x, y) {
        ...
        const anchor = this.anchorElement;
        const offset = anchor.offsetWidth / 2;
        const offsetX = position.x - offset + 5;
        const offSetY = position.y - offset - 5;
        const xPercent = offsetX / this._canvas.width * 100;
        const yPercent = offSetY / this._canvas.height * 100;
