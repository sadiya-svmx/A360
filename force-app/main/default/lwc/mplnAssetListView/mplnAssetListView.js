...
import MAINTENANCE_PLAN_OBJECT from '@salesforce/schema/SM_MaintenancePlan__c';

import getAssetList
    from '@salesforce/apex/MPLN_MaintenancePlanTemplate_LS.getAssetList';
import getMatchingAssetsFromTemplate
    from '@salesforce/apex/MPLN_ConditionBasedMPlan_LS.getMatchingAssetsFromTemplate';
import getMaintenanceAssets
    from '@salesforce/apex/MPLN_ConditionBasedMPlan_LS.getMaintenanceAssets';
...
import labelIncludeChildAssets from '@salesforce/label/c.Label_IncludeChildAssets';
import labelAssetQualificationCriteria from '@salesforce/label/c.Label_AssetQualificationCriteria';
import selectorModalTitle from '@salesforce/label/c.Label_SelectorModalTitle';
import helpTextAssetQualificationCriteria from
        '@salesforce/label/c.HelpText_AssetQualificationCriteria';
...
const i18n = {
    ...
    includeChildAssets: labelIncludeChildAssets,
    assetQualificationCriteria: labelAssetQualificationCriteria,
    selectorModalTitle: selectorModalTitle,
    helpTextAssetQualificationCriteria: helpTextAssetQualificationCriteria
}
...
export default class MplnAssetListView extends LightningElement {
    ...
    isSourceRecordAsset = false;
    openFilterCriteria = false;
    ...
    selectedExpression;
    includeChildAssetRecords = false;
    ...
    connectedCallback () {
        ...
        if (this.recordId && this.recordId.substring(0, 3) === ASSET_PREFIX) {
            this.isSourceRecordAsset = true;
        }
        this.fetchRecords();
    }
    ...
    fetchRecords () {
        ...
        .then(result => {
            if (result && result.data) {
                if (result.data === MAINTENANCE_PLAN_OBJECT.objectApiName) {
                    this.getMaintenanceAssetsFromPlan();
                } else {
                    this.getMaintenanceAssetsFromTemplate();
                }
    ...
    getMaintenanceAssetsFromPlan () {
        getMaintenanceAssets({ recordId: this.recordId })
        .then(result => {
    ...
    getMaintenanceAssetsFromTemplate () {
        getMatchingAssetsFromTemplate({
            requestJson: JSON.stringify({
                templateId: this.templateId,
                sourceRecordId: this.recordId,
                includeChildAssets: this.includeChildAssetRecords,
                assetCriteria: this.selectedExpressionId
            })
        }).then(result => {
            if (!verifyApiResponse(result)) {
                this.error = result.message;
            ...
            .then(expressionData => {
                if (expressionData) {
                    this.selectedExpression = {
                        label: expressionData.name,
                        value: expressionData.id
                    };
                }
            });
            if (result.data.length === 0) {
    ...
    populateMaintenanceAssetList (data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            tempLocalMaintenanceAssetRecords.push({
                ...
                'workTypeUrl': data[i].workTypeId ? sfdcBaseUrl + data[i].workTypeId : null
            });
    ...
    handleNext () {
        ...
        this.bottomErrorMessage = null;
        // eslint-disable-next-line @lwc/lwc/no-api-reassignments
        this.maintenanceAssetRecords = this.localMaintenanceAssetRecords;
        const navigateNextEvent = new FlowNavigationNextEvent();
        this.dispatchEvent(navigateNextEvent);
    }
    handleOpenFilterCriteria () {
       this.openFilterCriteria = true;
    }
    ...
    handleCloseModal () {
        this.openFilterCriteria = false;
    }
    handleIncludeChildAssets (event) {
        this.apiInProgress = true;
        this.includeChildAssetRecords = event.target.checked;
        this.fetchRecords();
    }
}