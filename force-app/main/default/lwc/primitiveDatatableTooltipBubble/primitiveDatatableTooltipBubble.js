...
import { classSet, classListMutation, keyCodes } from 'c/utils';
import labelCloseButtonAssistiveText from '@salesforce/label/c.AltText_Close';
...
export default class PrimitiveDatatableTooltipBubble extends LightningElement {
    @track _inDom = false;
    @track _visible = false;
    @track _align = DEFAULT_ALIGN;
    ...
    handleBlur = evt => {
        ...
        // If the element with the focus is inside the bubble, 
        // then the component as a whole was not blurred.
        const isValidBlur =
    ...
    handleCloseButtonKey = event => {
        ...
        if (keysToClose.indexOf(event.keyCode) !== -1) {
            this.dispatchCloseButtonEvent();
        }
    };
